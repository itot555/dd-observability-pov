...
# Python EC2 の /home/ubuntu/.env に直接書き込む（null_resource.deploy_env）。
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# Deploy /home/ubuntu/.env to Python EC2 via remote-exec
# (validation environment — credentials stored locally on EC2 only)
#------------------------------------------------------------------------------
...
resource "null_resource" "deploy_env" {
  ...
  depends_on = [
    null_resource.upload_python_app,
  ]
}
